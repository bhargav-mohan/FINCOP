...
def test_razorpay_fixture_closes_narration_only_settlements():
    """Payment id lives only in the bank memo. Unique identity must close, still validator-gated."""
    ingested = ingest_zip(FIXTURE_ZIP)
    ...
    config = ReconConfig(num_records=max(len(ingested.batch.ledger), 50), date_lag_days=5, use_llm=False)
    result = reconcile(ingested.batch.all_records, config)
    memo_ids = {r.id for r in memo_banks}
    engine_closed = memo_ids & result.closed_record_ids
    bench = orchestrate(result, config)
    assert memo_ids <= result.closed_record_ids
    for bank in result.records:
        if bank.id not in memo_ids:
            continue
        members = [
            r
            for r in result.records
            if r.id in result.closed_record_ids
            and compact_reference(r.reference) == compact_reference(bank.reference)
        ]
    ...
    if not engine_closed:
        ...
        assert kpis["exceptions_reduced"] >= 5
